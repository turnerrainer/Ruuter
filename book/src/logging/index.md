...
| **DSL step** | INFO `Executed` (gated by `logging.log_step_executions`, on by default) / DEBUG (`step_timing`) / INFO `dsl.log` (from `log:` step) | Every completed step for the INFO trail; `log:` step body | `dsl.step`, `dsl.step.type`, `duration_ms`, `dsl.next.step`, plus a rendered `attrs` field with step-type-specific context (see [Configuration reference](./configuration.md#log_step_executions)) |
