...
| `RUUTER_LOG_FORMAT`             | Log output format: `text`, `pretty` (ANSI colours + Unicode markers, local dev only), or `json`. Overrides `logging.format`. Full ref: [Logging](../logging/index.md) | value of `logging.format` in config (default `text`) |
