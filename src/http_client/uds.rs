...
pub async fn request_over_unix(
    ...
    let fut = async move {
        ...
        // upstreams silently bound `body` as `None`, which surfaced
        // in the DSL as JSON null and dropped the payload.
        let parsed_body: Option<Value> =
            crate::http_client::decode_response_body(&body_bytes, &response_headers);
